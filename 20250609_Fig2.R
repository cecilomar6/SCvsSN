...
plot <- inner_join(data.frame(res.sn) %>% rownames_to_column("gene"),
  ...
  geom_point(data = . %>% mutate(sumlog = log2FoldChange.sc + log2FoldChange.sn) %>% 
               arrange(sumlog) %>% slice(1:5),
             size = 0.1, alpha = 1, color = "black")+
  geom_point(data = . %>% mutate(sumlog = log2FoldChange.sc + log2FoldChange.sn) %>% 
               arrange(desc(sumlog)) %>% slice(1:5),
             size = 0.1, alpha = 1, color = "black")+
  ...
  ggrepel::geom_text_repel(data = . %>% mutate(sumlog = log2FoldChange.sc + log2FoldChange.sn) %>% 
                             arrange(sumlog) %>% slice(1:5),
...
ggsave(filename = "SNvsSC/plots/Fig2/20250609_Fig2_SNvsSC_DEcorr.tiff",
       plot = plot,
       width = 1200,
       height = 1200,
       units = "px")
...
writexl::write_xlsx(PC1.gsea@result, "SNvsSC/plots/Fig2/20250609_PC1_GSEA_REACTOME.xlsx")
