############### 20260206. Supplementary figures #############################
...
pData(cds) %>% 
  ...
  mutate(sample_n = n()) %>% 
  dplyr::filter(celltype_main == "Epithelial cells") %>% 
  group_by(technique, sample_n, sample.id, condition) %>% 
  summarise(lineage_n = n(),
            perc = (lineage_n/sample_n)*100) %>% 
  distinct() %>% 
  writexl::write_xlsx(path = "SNvsSC/1. online submission documents/supporting data values/20251002_SCvsSN_epithelial_means.xlsx")
...
cell_metadata <- data.frame(epi@meta.data) %>% 
  dplyr::filter(technique == "SN",
                annotation %in% c("AT2", "Cdkn1a+ AT1", "AT1"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(epi), rownames(cell_metadata))])
...
cell_metadata <- data.frame(epi@meta.data) %>% 
  dplyr::filter(technique == "SC",
                annotation %in% c("AT2 cells", "Krt8 ADI", "AT1 cells", "Activated AT2 cells"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(epi), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
...
writexl::write_xlsx(gsea.list,
                    "SNvsSC/supplementary files/20260218_GSEA_Alveolar_epi.xlsx")
...
ggsave(filename ="SNvsSC/plots/supplementary/20260219_Epi_GSEA_similar.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Epi_GSEA_unique.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Epi_GSEA_contradictory.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
pData(cds) %>% 
  ...
  summarise(lineage_n = n(),
            perc = (lineage_n/sample_n)*100) %>% 
  distinct() %>% 
  writexl::write_xlsx(path = "SNvsSC/1. online submission documents/supporting data values/20251002_SCvsSN_endothelial_means.xlsx")
...
cell_metadata <- data.frame(endo@meta.data) %>% 
  dplyr::filter(technique == "SN",
                annotation %in% c("EC", "gCAP"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(endo), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
colnames(indicator_matrix) <- levels(cell_to_sample)
matrix <- endo@assays$RNA$counts
matrix <- matrix[,colnames(matrix) %in% rownames(cell_metadata)]
all(colnames(matrix) == rownames(cell_metadata))
pseudo.sn <- matrix %*% indicator_matrix
pseudo.sn <- as.matrix(pseudo.sn)
...
cell_metadata <- data.frame(endo@meta.data) %>% 
  dplyr::filter(technique == "SC",
                annotation %in% c("Vcam1+ VECs", "VECs"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(endo), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
...
writexl::write_xlsx(gsea.list,
                    "SNvsSC/supplementary files/20260218_GSEA_Endo.xlsx")
...
ggsave(filename ="SNvsSC/plots/supplementary/20260219_Endo_GSEA_similar.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Endo_GSEA_unique.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Endo_GSEA_contradictory.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
pData(cds) %>% 
  ...
  summarise(lineage_n = n(),
            perc = (lineage_n/sample_n)*100) %>% 
  distinct() %>% 
  writexl::write_xlsx(path = "SNvsSC/1. online submission documents/supporting data values/20251002_SCvsSN_mesenchymal_means.xlsx")
...
pData(cds) %>% 
  ...
  summarise(lineage_n = n(),
            perc = (lineage_n/sample_n)*100) %>% 
  distinct() %>% 
  writexl::write_xlsx(path = "SNvsSC/1. online submission documents/supporting data values/20251002_SCvsSN_myeloid_means.xlsx")
...
cell_metadata <- data.frame(myelo@meta.data) %>% 
  dplyr::filter(technique == "SN",
                annotation %in% c("AM"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(myelo), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
colnames(indicator_matrix) <- levels(cell_to_sample)
matrix <- myelo@assays$RNA$counts
matrix <- matrix[,colnames(matrix) %in% rownames(cell_metadata)]
all(colnames(matrix) == rownames(cell_metadata))
pseudo.sn <- matrix %*% indicator_matrix
pseudo.sn <- as.matrix(pseudo.sn)
...
cell_metadata <- data.frame(myelo@meta.data) %>% 
  dplyr::filter(technique == "SC",
                annotation %in% c("AM (PBS)", "AM (Bleo)"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(myelo), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
...
writexl::write_xlsx(gsea.list,
                    "SNvsSC/supplementary files/20260218_GSEA_Myelo.xlsx")
...
ggsave(filename ="SNvsSC/plots/supplementary/20260219_Myelo_GSEA_similar.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Myelo_GSEA_unique.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Myelo_GSEA_contradictory.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
pData(cds) %>% 
  ...
  summarise(lineage_n = n(),
            perc = (lineage_n/sample_n)*100) %>% 
  distinct() %>% 
  writexl::write_xlsx(path = "SNvsSC/1. online submission documents/supporting data values/20251002_SCvsSN_lymphoid_means.xlsx")
...
cell_metadata <- data.frame(lympho@meta.data) %>% 
  dplyr::filter(technique == "SN",
                annotation %in% c("TC"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(lympho), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
colnames(indicator_matrix) <- levels(cell_to_sample)
matrix <- lympho@assays$RNA$counts
matrix <- matrix[,colnames(matrix) %in% rownames(cell_metadata)]
all(colnames(matrix) == rownames(cell_metadata))
pseudo.sn <- matrix %*% indicator_matrix
pseudo.sn <- as.matrix(pseudo.sn)
...
cell_metadata <- data.frame(lympho@meta.data) %>% 
  dplyr::filter(technique == "SC",
                annotation %in% c("T-lymphocytes", "T cell subset", "Themis+ T-lymphocytes"))
unique(cell_metadata$annotation)
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(lympho), rownames(cell_metadata))])
cell_to_sample <- cell_to_sample[!is.na(cell_to_sample)]
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
...
ggsave(filename ="SNvsSC/plots/supplementary/20260219_Lymph_GSEA_similar.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Lymhp_GSEA_unique.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/supplementary/20260219_Lymph_GSEA_contradictory.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
