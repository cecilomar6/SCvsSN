...
data@meta.data %>% 
  ...
  writexl::write_xlsx(path = "SNvsSC/1. online submission documents/supporting data values/20251002_SCvsSN_UMI_Feature_means.xlsx")
...
plot <- data.frame(pData(cds)) %>% 
  ...
  scale_color_manual(values = c("lightskyblue", "steelblue3",
                                "gold1", "darkgoldenrod2", "goldenrod4",
                                "indianred1", "indianred3", 
                                "mediumorchid3","mediumorchid4", "grey70"))

plot

ggsave(filename = "SNvsSC/plots/Fig2/20260218_Fig2_SNvsSC_UMAP_celltype_mid.tiff",
...
ggsave(filename = "SNvsSC/plots/Fig2/20260218_Fig2_SNvsSC_celltype_legend.pdf",
...
plot <- rbind(pData(sn)[, colnames(pData(sn)) %in% c("technique", "celltype_main", "celltype_mid", "condition")],
  ...
  scale_fill_manual(values =c("lightskyblue", "steelblue3",
                              "gold1", "darkgoldenrod2", "goldenrod4",
                              "indianred1", "indianred3", 
                              "mediumorchid3","mediumorchid4", "grey70"))+
  theme_classic()+
...
ggsave(filename = "SNvsSC/plots/Fig2/20260218_Fig2_SNvsSC_Proportions.pdf",
       plot = plot,
       width = 1200,
       height = 3000,
       units = "px")
...
plot <- rbind(pData(sn)[, colnames(pData(sn)) %in% c("technique", "celltype_main", "celltype_mid", "condition")],
  ...
  dplyr::select(celltype_mid, technique, ratio,condition) %>% 
  distinct() %>% 
  ggplot(aes(y = ratio, x = technique, fill = celltype_mid))+
  geom_col()+
  ...
  theme(aspect.ratio = 1.62,
        ...
        strip.background = element_blank(),
        axis.title.y = element_text(margin = margin(t = 10), size = 18))+
  guides(fill = guide_legend(ncol = 1))+
...
cell_metadata <- data.frame(pData(sn))
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(sn), rownames(cell_metadata))])
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
colnames(indicator_matrix) <- levels(cell_to_sample)
pseudo.sn <- counts(sn) %*% indicator_matrix
...
cell_metadata <- data.frame(pData(sc))
cell_to_sample <- factor(cell_metadata$sample.id[match(colnames(sc), rownames(cell_metadata))])
indicator_matrix <- sparse.model.matrix(~ 0 + cell_to_sample)
colnames(indicator_matrix) <- levels(cell_to_sample)
pseudo.sc <- counts(sc) %*% indicator_matrix
...
writexl::write_xlsx(data.frame(res.sn) %>% rownames_to_column("gene") %>% arrange(padj),
                    "SNvsSC/supplementary files/20260218_DE_SN_7vs0.xlsx")
...
writexl::write_xlsx(data.frame(res.sc) %>% rownames_to_column("gene") %>% arrange(padj),
                    "SNvsSC/supplementary files/20260218_DE_SC_7vs0.xlsx")
...
writexl::write_xlsx(gsea.list,
                    "SNvsSC/supplementary files/20260218_GSEA.xlsx")
...
ggsave(filename = "SNvsSC/plots/Fig2/20260219_Fig2_SNvsSC_GSEA_similar.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/Fig2/20260219_Fig2_SNvsSC_GSEA_unique.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/Fig2/20260219_Fig2_SNvsSC_GSEA_diverging.pdf",
       plot = plot,
       width = 4000,
       height = 1800,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/Fig2/20260218_Fig2_SNvsSC_DEcorr.tiff",
       plot = plot,
       width = 1200,
       height = 1200,
       units = "px")
...
pseudo.all <- full_join(pseudo.sn %>% data.frame() %>% rownames_to_column("gene"),
                        pseudo.sc %>% data.frame() %>% rownames_to_column("gene"),
                        by = "gene") %>% 
...
colnames(pseudo.all) <- coldata.all$sample.id
...
plot <- PC1.gsea@result %>% 
  ...
  scale_color_manual(values = c("#00BFC4", "grey60", "#F8766D"))+
  xlim(-3,5.3)+
...
ggsave(filename = "SNvsSC/plots/Fig2/20250826_Fig2_SNvsSC_GSEA_volcano.pdf",
       plot = plot,
       width = 2000,
       height = 1800,
       units = "px")
