...
plot <- data.frame(pData(sn)) %>% 
  ...
  ggrepel::geom_text_repel(data = . %>% group_by(annotation) %>%
                             summarize(UMAP1 = mean(UMAP1),
                                       UMAP2 = mean(UMAP2)),
                           aes(x = UMAP1, y = UMAP2, label = annotation),
                           color = "black", size = 3,
                           inherit.aes = FALSE)

plot

ggsave(filename = "SNvsSC/plots/Fig1/20260319_Fig1_UMAP_celltype.tiff",
       plot = plot,
       width = 1500,
       height = 1500,
       units = "px")
...
ggsave(filename = "SNvsSC/plots/Fig1/20260319_Fig1_SN_UMAP_celltype_mid.tiff",
...
ggsave(filename = "SNvsSC/plots/Fig1/20260218_Fig1_SN_celltype_legend.pdf",
...
plot
ggsave(filename = "SNvsSC/plots/Fig1/20260219_Fig1_SN_Proportions.pdf",
       plot = plot+ theme(legend.position = "none"),
       width = 2000,
       height = 1200,
       units = "px")

## REVIEW: Proportions in boxplot ----

data.frame(pData(sn)) %>%  
  group_by(sample.id) %>% 
  mutate(all_n = n()) %>% 
  group_by(celltype_mid, condition, sample.id) %>% 
  mutate(n = n(),
         ratio = n/all_n) %>% 
  select(celltype_mid, ratio, condition, sample.id) %>% 
  distinct() %>% 
  ggplot(aes(x = condition, y = ratio, fill = celltype_mid))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(width = 0.1)+
  facet_wrap(~ celltype_mid, scales = "free", nrow = 2)+
  theme_bw()+
...
### Statistics with propeller ----

library(speckle)
library(limma)

# Trying propeller for statistical analysis
# Using the anova function since we have 3 groups

metadata <- data.frame(pData(sn)) %>% 
...
design <- model.matrix(metadata$sample ~ 0 + condition)

props <- getTransformedProps(
  clusters = factor(sn$celltype_mid),
  sample = sn$sample.id,
  transform = "logit"
)

stat.res <- propeller.anova(props,
                            design = design,
                            coef = c(1,2,3,4),  # which coefficients to test
                            robust = TRUE,
                            trend = FALSE,
                            sort = TRUE)

# Even after using propeller and with very clear differences we didn't reach 
# statistic significance in any compartment.
